...
- **LLM providers supported**: OpenAI, Azure OpenAI, Gemini, Groq, Anthropic, Ollama, Grok, AWS Bedrock and LM Studio backends are supported. OpenAI key is probably still needed for embeddings in many tools. Don't forget to load an embedding model when using LM Studio.
